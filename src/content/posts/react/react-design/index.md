...
current node 对应于当前的真实 dom ，它最大的作用是：当他和真实 dom 存在差异时，就会触发下一阶段的处理。

而 workInProgress node 是总是在内存中更新的一个对象，我们会将下一帧需要更新的内容，首先应用到这个对象上，进行各种计算。

需要实际应用更新的时候，切换一下这两个 node 即可。

回到举例中，我们的调度器收到了更新队列，其中包含了两个 Update 对象。

调度中心能够找到触发了更新的组件，然后将这个组件标记为待更新状态。

实际上的操作就是：执行 js 代码，修改 workInProgress node 上某个子节点的标记属性。
...
回到举例中，调度器标记了组件为待更新状态后，会开始计算优先级。
...
调度器得到了要更新的内容之后，就可以应用到 workInProgress node 上。

它会修改 workInProgress node 上某个子节点的属性。
...
简单来说，调度器处理了上一阶段提供的更新任务，更新了 Fiber node，准备好了全部信息之后，触发下一阶段的执行。

下一阶段，在 react 中称为 render 阶段，中文翻译为渲染阶段，这里的渲染和浏览器中的 cssom 和 dom 渲染是两个概念。

注意：调度器在当前调度工作完成后，可能还有部分任务没有处理。这些任务，将会等待下一次调度中心的处理。
